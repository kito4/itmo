    ...
    public Object execute(String[] args, TextSender sender) {
        if (args.length == getAMOUNT_OF_ARGS()) {
            try {
                ...
                if (humanInfoInput.getNewHumanToInput().getImpactSpeed()> Config.getCollectionManager().returnDescending().get(Config.getCollectionManager().returnDescending().size()-1).getImpactSpeed()) { //big bruh
